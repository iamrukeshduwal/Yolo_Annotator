...
class LabelTool():
    ...
    def mouseClick(self, event):
        ...
        # Check if the click is within an existing bounding box for dragging
        else:
            ...
            if selected_bbox_id:
                self.drag_data = {'x': x, 'y': y, 'item': selected_bbox_id}
                self.mainPanel.tag_bind(selected_bbox_id, '<B1-Motion>', self.on_bbox_drag)

    
    def on_bbox_drag(self, event):
        x = self.mainPanel.canvasx(event.x)
        y = self.mainPanel.canvasy(event.y)
        bbox_id = self.drag_data['item']
        self.mainPanel.move(bbox_id, x - self.drag_data['x'], y - self.drag_data['y'])
        self.drag_data['x'] = x
        self.drag_data['y'] = y
